...
- `ollama.enabled`, `ollama.executable_path`, `ollama.models_directory`, `ollama.port`, and `ollama.disable_thinking` control the portable Ollama runtime. Portable Ollama defaults to thinking off for the agent path, which prevents small models from spending the full 64k window on unbounded reasoning; the request also pins `num_ctx` to the configured context and sends `think: false` through the managed custom-provider entry. The default model store is `ollamamodel\`; loose files placed there are not Ollama models until imported or pulled through Ollama.
